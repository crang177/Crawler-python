...
async def get_ts(get_ts_url,headers):
    ...
    async with aiohttp.ClientSession() as session :
        async with session.get(get_ts_url,headers=headers) as res:
            ...
            ts_download_list=[]
            for i in ts_list:
                ts_download_list.append("https://vod-trans-1.lgzk.whut.edu.cn"+i+".ts")
...
async def ts_download_all(ts_download_list) :
    ...
    for i in range(len(ts_download_list)):
        cor=downlao_one(ts_download_list[i],    str(i+1)+".ts",   sem)
...
async def main():
    ...
    url=input("输入m3u8的url： ")
    print("开始爬取")
    await get_ts(url,headers=headers)
    do_m3u8_url()
    merge(filename="1")
    delate_ts()
